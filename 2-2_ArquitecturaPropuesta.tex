...
La autonomía del Nodo de Consumo se sustenta en una rutina de control cíclica que se ejecuta localmente en el microcontrolador. La Figura~\ref{img:Diagrama-Flujo} esquematiza el flujo de operaciones que realiza el \ing{firmware} para gestionar el estado de la carga.
...
El diseño del sistema es agnóstico al \ing{hardware}, lo que significa que puede desplegarse tanto en servidores en la nube (VPS) como en computadoras de placa reducida (SBC) ubicadas en el mismo sitio de la microrred.
...
La lógica de interconexión se basa en el flujo de datos ascendente desde el \ing{hardware} hacia la interfaz de usuario. Cada contenedor Docker desempeña un rol específico en esta cadena de procesamiento:
