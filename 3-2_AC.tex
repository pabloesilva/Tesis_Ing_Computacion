...
Para la medición de la tensión de la microrred se emplea un módulo sensor basado en el transformador de señal ZMPT101B, seleccionado tanto por sus características técnicas como por su disponibilidad comercial. Este módulo genera una señal sinusoidal con un nivel de continua (\ing{offset}) y una amplitud proporcional a la tensión real medida. Antes de ser leída por la entrada analógica del microcontrolador, la señal atraviesa una etapa de adecuación que garantiza que sus valores se mantengan dentro del rango permitido por el ESP32, evitando sobrepasar el límite máximo de 3,3 V.
...
Con el objetivo de integrar la interfaz local en la caja de paso de 10×10 cm, se efectuó un mecanizado de la tapa para alojar tanto los pulsadores como el \ing{display}. La disposición de estos elementos fue seleccionada para garantizar accesibilidad al usuario, buena visibilidad y comodidad durante la utilización del sistema. Para asegurar un montaje firme y estético del \ing{display}, se diseñó un soporte específico mediante \ing{software} de modelado tridimensional y se imprimió en 3D utilizando material plástico de alta resistencia. Este soporte permite fijar el \ing{display} en la posición adecuada y facilita el ensamblaje final sin necesidad de herramientas adicionales.
...
La medición de la tensión de línea proveniente de la microrred se realiza mediante un muestreo síncrono de alta frecuencia. Se configuró un temporizador por \ing{hardware} para generar interrupciones a una frecuencia de 50 kHz. En cada interrupción, se dispara la conversión del ADC, asegurando que la frecuencia de muestreo sea un múltiplo exacto de la frecuencia fundamental de la red (50 Hz). Esto permite capturar un número entero de periodos de la señal, minimizando el error por efecto de ``ventaneo'' en el cálculo del valor eficaz (RMS).

El \ing{firmware} implementa un procedimiento de calibración al inicio para determinar el nivel de continua (\ing{offset}) introducido por el sensor ZMPT101B. Posteriormente, en tiempo de ejecución, cada muestra es corregida restando este \ing{offset}. El valor RMS se calcula procesando la suma de los cuadrados de las muestras adquiridas en una ventana de tiempo definida, aplicando luego la raíz cuadrada según la Ecuación~\ref{eq:rms}:
...
Para la coordinación con los NC, el \ing{firmware} gestiona el intercambio de mensajes de control sin conexión (\ing{connectionless}). El AC difunde periódicamente un mensaje de \textit{Disponibilidad}, que contiene el límite de corriente calculado para la red. A su vez, se mantiene a la escucha de los mensajes de \textit{Consenso} provenientes de los nodos, los cuales reportan su consumo actual y prioridad.
...
La interacción local se gestiona mediante la actualización condicional del \ing{display} OLED (\textit{Organic Light-Emitting Diode}). Para minimizar la carga de procesamiento, el contenido de la pantalla solo se redibuja cuando se detectan cambios significativos en los valores mostrados o cuando el usuario interactúa con los pulsadores.
